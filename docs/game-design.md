...
> **Fase actual:** Post-MVP (Fase D en operación)
> **Estado operativo técnico:** D.4a completado (guardrails/claims) · D.4b completado (observabilidad operativa de claims + hardening de concurrencia)
...
**Duración de un ciclo de loop completo:** 2–15 minutos (configurable por zona y decisión del jugador). Estado actual: 3 zonas con escalado de riesgo/recompensa y desbloqueo por nivel.
...
- Selecciona una zona disponible para su nivel (zonas de mayor riesgo se bloquean hasta progresar).
...
- **SAFE (default):** perfil conservador para sesiones estables. Menor payout de CC, XP y drops (especialmente rarezas altas). En catástrofe **no** se pierde equipo equipado.
- **HARD / DANGEROUS:** perfil de alto riesgo. Mayor payout y mejor acceso a materiales complejos (RARE/EPIC/LEGENDARY vía multiplicadores determinísticos server-side). En catástrofe se pierde todo el equipo snapshotteado en la run.

Regla de integridad: el modo se snapshottea en servidor al iniciar (`dangerConfig.runMode`) y jamás se decide en cliente.

### Buildcraft por sinergias/arquetipo (D.3.1)

- El loadout activo ahora puede activar **sinergias configurables** (familias + slots + rarezas).
- Las sinergias y el arquetipo se resuelven en servidor y aplican bonus con **caps explícitos**.
- Efectos actuales de D.3.1 impactan:
  - multiplicadores de loot/currency/xp,
  - resistencia de peligro,
  - `dangerLootBonus` y umbral de catástrofe.
- La UI solo muestra el resultado activo (sinergias/arquetipo); no calcula autoridad económica local.

### LiveOps semanal read-only (D.3.2)

- Se introduce overlay de evento activo + directivas semanales visibles en dashboard.
- El progreso se deriva desde datos existentes (`ExtractionResult`) y es solo informativo en este slice.
- No hay sistema de claim persistente todavía (queda para D.3b).

### Analytics de jugador (D.3.3)

- Se añade panel de métricas personales para feedback táctico:
  - tasa de éxito,
  - promedio CC/XP por extracción,
  - mix SAFE/HARD,
  - mejor zona por earnings.
- Objetivo de diseño: mejorar claridad de decisiones sin introducir bucles de recompensa nuevos.
...
### Parámetros de zona base (estado actual)
```
...
- Los items de equipo otorgan stats que afectan la run: `lootMultiplier`, `dangerResistance`, `xpMultiplier`, `backpackCapacity`.
...
### Desbloqueables de nivel (estado actual)
- Los niveles sí desbloquean acceso a zonas por `minLevel`.
- La progresión mantiene mejora numérica continua (recompensas y eficiencia de runs) más acceso gradual a contenido high-tier.
