...
export function InventorySlot({ item, isEquipped, isRunActive = false }: InventorySlotProps) {
  ...
  return (
    <Card className={cn('group relative flex flex-col items-center justify-between overflow-visible glass-panel border-l-4 transition-all hover:scale-[1.02] hover:shadow-[0_0_15px_rgba(0,243,255,0.2)] cyberpunk-box', rarityVisuals.borderClass, rarityVisuals.bgClass)}>
      ...
      <CardContent className="p-4 pt-4 flex flex-col items-center w-full">
        <div className="mb-3 flex w-full items-start justify-between gap-2">
          ...
          {isEquipped && (
            <Badge variant="outline" className="border-cyan-400/50 bg-cyan-500/10 px-1.5 py-0 text-[10px] uppercase tracking-widest text-cyan-300">
              Equipado
            </Badge>
          )}
        </div>
